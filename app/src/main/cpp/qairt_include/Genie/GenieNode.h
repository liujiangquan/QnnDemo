...
typedef GenieNode_TextOutput_SentenceCode_t GenieNode_SentenceCode_t;

typedef enum {
  GENIE_NODE_TEXT_GENERATOR_TEXT_INPUT            = 0,
  GENIE_NODE_TEXT_GENERATOR_EMBEDDING_INPUT       = 1,
  GENIE_NODE_TEXT_GENERATOR_TEXT_OUTPUT           = 2,
  GENIE_NODE_TEXT_GENERATOR_TOKEN_INPUT           = 3,
  GENIE_NODE_TEXT_GENERATOR_TOKEN_OUTPUT          = 4,
...
/**
 * @brief A client-defined callback function to handle text-generator token responses.
 *
 * @param[in] response Array of output token IDs. May be NULL on a terminal sentence code.
 *
 * @param[in] numTokens Number of tokens in the response array.
 *
 * @param[in] sentenceCode The sentence code related to the response.
 *
 * @param[in] outputConfig Currently unused; always NULL. Reserved for future use.
 *
 * @param[in] userData The userData field provided to GeniePipeline_execute.
 *
 * @return None
 *
 */
typedef void (*GenieNode_TokenOutput_Callback_t)(const uint32_t* response,
                                                 const uint32_t numTokens,
                                                 const GenieNode_SentenceCode_t sentenceCode,
...
/**
 * @brief A function to setCallback for text generator node token output.
 *
 * @param[in] nodeHandle A node handle.
 *
 * @param[in] nodeIOName Node IO name.
 *
 * @param[in] callback A user callback function for raw token output.
 *
 * @return Status code:
 *         - GENIE_STATUS_SUCCESS: API call was successful.
 *         - GENIE_STATUS_ERROR_INVALID_HANDLE: Node handle is invalid.
 ...
 */
GENIE_API
Genie_Status_t GenieNode_setTokenCallback(const GenieNode_Handle_t nodeHandle,
                                          const GenieNode_IOName_t nodeIOName,
                                          const GenieNode_TokenOutput_Callback_t callback);
