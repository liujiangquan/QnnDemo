...
```json
{
  ...
  // lm_head 与 embed_tokens 共享权重（tie_word_embeddings=true），不需要 tensor.output_weight
}
```
...
`operation.rope_num_rotations = head_dim = hidden_size / num_attention_heads`

- Qwen2.5-3B：`2048 / 16 = 128`
- Qwen2.5-7B：`3584 / 28 = 128`（与 SDK qwen2.5-7b.json 一致 ✓）

参照：llama3.2-3b.json `embedding=3072, heads=24, rope_num_rotations=128` → `3072/24=128` ✓
