...
# yolo* 走 YoloE2ETest（YOLO26-pose 静态图场景），其它走 BackendE2ETest（CNN 场景）
...
elif [[ "$filter_test" == yolo* ]]; then
    CLASS="com.breeze.qnn.YoloE2ETest"
    if [ "$filter_test" = "yolo" ]; then filter_test=""; fi
fi
test_arg=""
